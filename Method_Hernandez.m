...
clear;
addpath(genpath('./lib'));
fname = 'C:\Users\Willi\ucloud\BaseAngles\OpenSimInSlicer\Right_Hernandez.mrk.json';
side = 'R';
...
u = data.markups.controlPoints(hjcI).position - data.markups.controlPoints(gtI).position;
% shaftAxis = data.markups.controlPoints(psI).position - data.markups.controlPoints(dsI).position;
v = data.markups.controlPoints(lcI).position - data.markups.controlPoints(mcI).position;

CosTheta = max(min(dot(u,v)/(norm(u)*norm(v)),1),-1);
AVA_inPlanePerpendicularToShaft = real(acosd(CosTheta));

AVA = atan2d(norm(cross(u,v)),dot(u,v))
